...
def initial_population():
    ...
    numpy.save('training_data_1.py', training_data_save)
...
def play_games(model):
    ...
    for each_game in range(100):
        ...
        for _ in range(goal_steps):
            ...
            #sometimes predict converge on thing

            new_observation, reward, done, info = env.step(action)
            prev_obs = new_observation
            game_memory.append([new_observation, action])
            score += reward
            #print('loop:', score,' ', reward)
            if done:
...
#training_data = numpy.load('training_data_1.py.npy')
#print(training_data)
model = train_model(training_data)
#model.save('test_model_1.model')

#with open('test_file_1.txt') as file:  
#    length = file.read()
#model = neural_network_model(4)

#model.load('200.model')
#model.load('test_model_1.model')
